...
namespace GFX
{
    ...
    void BinaryStream::Write(const std::string &value, size_t length)
    {
        int numBytes = BitConverter::GetBytes(value, length, buffer, writeOffset);
        AdvanceWriteOffset(numBytes);
    }

    void BinaryStream::Write(const void *buffer, size_t length)
    {
        std::memcpy(&this->buffer[writeOffset], buffer, length);
    ...
    {
        std::memcpy(buffer, &this->buffer[readOffset], length);
        AdvanceReadOffset(length);
    }
