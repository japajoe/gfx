...
public:
    FrameBufferObject();
    FrameBufferObject(uint32_t width, uint32_t height, bool multiSample, bool hdr);
    ...
    void Blit(const FrameBufferObject &fbo);
    void Blit(uint32_t fbo, uint32_t width, uint32_t height);
    GLuint GetId() const;
